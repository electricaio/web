...
describe('api keys reducer', () => {
  ...
  describe('refresh api key', () => {
    it('handles refresh api key request', () => {
      expect(apiKeysReducer(initialState, { type: ApiKeysTypes.REFRESH_KEY })).toEqual({
        ...initialState,
        loading: true,
      });
    });

    it('handles successful 1 refresh api key', () => {
      expect(
        apiKeysReducer(initialState, {
          type: ApiKeysTypes.REFRESH_KEY_SUCCESS,
          payload: {
            id: 1,
            userId: 1,
            name: 'test2',
